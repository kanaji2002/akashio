...
# データの読み込み
water_temp_df = pd.read_csv("scr_csv/suionn-sum.csv", parse_dates=['datetime'], dayfirst=True)
s_target = 'suionn'
...
# モデルの構築
model = ()
model=load_model('predict_py/model_from_suionn6.h5')
# ここから変更--------
...
def predict_temperature(model, specified_data):
    ...
    if specified_date in data.index:
        ...
        for day in range(60):
            ...
            if day == 0:
                ...
                suionn_temp.append(predicted_value_specified_date[-1, 0] )
    ...
    merged_df = pd.merge(pred_df,results_df[['datetime', 'suionn']], on='datetime', how='left', suffixes=('', '_new'))

# 結合後のnissyaryou_new列で元のnissyaryou列を更新します。
    merged_df['suionn'] = merged_df['suionn_new']

# 不要になったnissyaryou_new列を削除します。
    merged_df.drop(columns='suionn_new', inplace=True)

    merged_df.to_csv('predict_csv\predict_data.csv',index=False)
    print("suionn yosoku OK!")


# 強制的な日付指定
#specified_date = '2024/3/7'
