...
def predict_temperature(model, specified_data,weather_df):
    ...
    # print(type(specified_data))
    # temp_df=pd.DataFrame(data=datetime,columns='nissyaryou')
    if specified_date in data.index:
        ...
        for day in range(7):
            ...
            if day == 0:
                ...
                datetime_temp.append(current_date)
                nissyaryou_temp.append(predicted_value_specified_date[-1, 0]+weather_df.loc[day, 'nissyaryou'] )
                results_df = pd.DataFrame({
                'datetime': datetime_temp,
                'nissyaryou': nissyaryou_temp
                })
                ...
                datetime_temp.append(next_date_srt)
                nissyaryou_temp.append(predicted_value_specified_date[-1, 0]+weather_df.loc[day, 'nissyaryou'] )
                results_df = pd.DataFrame({
                'datetime': datetime_temp,
                'nissyaryou': nissyaryou_temp
    ...
    # print(results_df)
    # df1とdf2を'datetime'列を基準にして結合します。この例では左外部結合を使用しています。
# df2のnissyaryou列を一時的に別名で結合してから、df1のnissyaryou列を更新します。
    merged_df = pd.merge(pred_df,results_df[['datetime', 'nissyaryou']], on='datetime', how='left', suffixes=('', '_new'))

# 結合後のnissyaryou_new列で元のnissyaryou列を更新します。
    merged_df['nissyaryou'] = merged_df['nissyaryou_new']

# 不要になったnissyaryou_new列を削除します。
    merged_df.drop(columns='nissyaryou_new', inplace=True)

    merged_df.to_csv('predict_csv\predict_data.csv',index=False)

    print("nissyaryou yosoku OK!")
